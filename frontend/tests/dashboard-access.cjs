  ...
  try {
    for (const scenario of ['platform-admin', 'workspace-member', 'no-workspace']) {
      ...
      await page.route('**/api/v1/**', async route => {
        ...
        else if (url.pathname.endsWith('/analytics/dashboard/')) json = { leads: { total_leads: 0, new_leads_today: 0, converted_leads: 0, open_conversations: 0, lead_to_booking_conversion_rate: 0 }, bookings: {}, date_range: {}, leads_timeseries: [], channels: [], recent_leads: [], activities: [] };
